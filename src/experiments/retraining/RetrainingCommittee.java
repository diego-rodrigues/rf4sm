...
public class RetrainingCommittee {
	...
	public static void main(String[] args) {
		...
		for (randSeed = 0; randSeed < 30; randSeed++){
			...
			String logFileName = EXPERIMENTS + "logs/RetrainingCommittee/"+ domain + "/" + MIN_AGREEMENT + "-" + domain + "-" + NUMBER_OF_TREES_COMMITTEE + "-" + Integer.toString(randSeed) + ".log.txt";
			try {
				pw = new PrintWriter(new File(logFileName));
				pw.printf("\t\t\t\tCommittee(%d):Baseline\t\t\t\n",NUMBER_OF_TREES_COMMITTEE);
...
//			Functions.createRandomTrainingAndTest(trainingInstances, testInstances, allInstances, trainingPerc, r);			//Test#1
			...
			try {
				...
				for (int round = 0; (round < 5) && (unlabeledInstances.numInstances() > 0); round++){
					...
					Vote rf = new Vote();
					committeeTrees = Functions.decideBestTrees(RFTrees, trainingInstances, NUMBER_OF_TREES_COMMITTEE, Functions.F1SCORE);
			...
			pw.printf("Number of Random Forest trees (Committee): %d\n" +
