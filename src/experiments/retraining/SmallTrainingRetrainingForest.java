...
/**
 * Na pasta SmallTrainingRetrainingForest estão os resultados que utilizam 10% de treino aleatório com 1% positivo.
 * Na pasta SmallTrainingRetrainingForest2 estão os resultados que utitlizam 30 instâncias de treino (15 positivas/15 negativas).
 * @author diego
 *
 */
public class SmallTrainingRetrainingForest {
	...
	public static void main(String[] args) {
		...
		for (randSeed = 0; randSeed < 30; randSeed++){
			...
			String logFileName = EXPERIMENTS + "logs/SmallTrainingRetrainingForest/"+ domain + "/" + MIN_AGREEMENT + "-" + domain + "-" + NUMBER_OF_TREES_COMMITTEE + "-" + Integer.toString(randSeed) + ".log.txt";
			try {
...
//			Functions.createCustomTrainingAndTest(trainingInstances, testInstances, allInstances, TRAINING_PERCENTAGE, 10, r);			//Test#2
			...
			try {
				...
				for (int round = 0; (round < 10) && (unlabeledInstances.numInstances() > 0); round++){
					...
					committeeTrees = Functions.decideBestTrees(RFTrees, trainingInstances, NUMBER_OF_TREES_COMMITTEE, f1Scores);
			...
			pw.printf("\n----------------------------------------\nSmall Training Retraining Forest\n");
